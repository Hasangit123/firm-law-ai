...
const markdownStyles = `
  .markdown-body { font-family: 'Georgia', serif; font-size: 14px; line-height: 1.8; color: rgba(245,240,232,0.8); }
            ...
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px'}}>
              {documentTypes.map(doc => (
                <div key={doc.id} onClick={() => selectDoc(doc)}
                  ...
                  <h3 style={{fontSize: '15px', fontWeight: 600, color: '#ffffff', marginBottom: '6px', fontFamily: 'Cormorant Garamond, serif'}}>{doc.title}</h3>
                  ...
                  <div key={field.id}>
                    ...
                    {field.id.includes('details') || field.id.includes('facts') || field.id.includes('statements') || field.id.includes('grounds') || field.id.includes('information_sought') ? (
                      <textarea value={formData[field.id] || ''} onChange={e => handleInput(field.id, e.target.value)}
                        placeholder={field.placeholder} rows={4} style={{...inputStyle, resize: 'none', lineHeight: 1.7}} />
                    ) : (
        ...
        {document && (
          <div>
            ...
            <div style={{background: 'var(--black-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px'}}>
              ...
              <div className="markdown-body" style={{background: 'rgba(255,255,255,0.03)', padding: '20px', borderRadius: '8px', border: '1px solid var(--border)'}}>
